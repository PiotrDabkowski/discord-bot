...
class LoginModal(miru.Modal):
    apikey = miru.TextInput(label="API key", placeholder="Enter your API key here!", style=hikari.TextInputStyle.SHORT, required=True, max_length=35)
    async def callback(self, ctx: miru.ModalContext) -> None:
        ...
        if r.status_code == 200:
            with open(f"{ctx.user.id}.txt", 'w') as validate:
                ...
                validate.close()
            await ctx.edit_response(f"API key validated ✅! Welcome {ctx.user.username}!")
